<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.pingjiatupian" label="评价图片" prop="pingjiatupian">
          <file-upload
          tip="点击上传评价图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.pingjiatupian" label="评价图片" prop="pingjiatupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.pingjiatupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="部门账号" prop="bumenzhanghao">
          <el-select :disabled="ro.bumenzhanghao" @change="bumenzhanghaoChange" v-model="ruleForm.bumenzhanghao" placeholder="请选择部门账号">
            <el-option
                v-for="(item,index) in bumenzhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.bumenzhanghao" label="部门账号" prop="bumenzhanghao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.yonghuming!=''&&json.yonghuming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gonganbumen/bumenzhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.bumenzhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    bumenzhanghaoChange () {
      this.$http({
        url: `follow/gonganbumen/bumenzhanghao?columnValue=`+ this.ruleForm.bumenzhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `pingjiajianyi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.pingjiajianyiCrossAddOrUpdateFlag = false;
						 this.parent.search();
